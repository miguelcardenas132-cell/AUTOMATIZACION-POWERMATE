...
const CONFIG = {
  ...
  ASEGURADO_PREFIJO: 'Asegurado ' // columnas "Asegurado 1".."Asegurado 5"
};
...
function construirTokens_(fila, encabezados, valores, obtenerValor) {
  ...
  for (let i = 1; i <= CONFIG.MAX_ASEGURADOS; i++) {
    const valor = obtenerValorSeguro_(encabezados, valores, CONFIG.ASEGURADO_PREFIJO + i);
    if (valor && valor.toString().trim() !== '') asegurados.push(valor.toString().trim());
  }
  ...
  const primaMaster = Number(obtenerValor(H.PRIMA_MASTER)) || 0;
  const primaSmart = Number(obtenerValor(H.PRIMA_SMART)) || 0;
  const primaElite = Number(obtenerValor(H.PRIMA_ELITE)) || 0;
  const primaPremium = Number(obtenerValor(H.PRIMA_PREMIUM)) || 0;

  const folio = 'COT-' + fila + '-' + Utilities.formatDate(hoy, Session.getScriptTimeZone(), 'yyyyMMdd-HHmmss');

  return {
    folio: folio,
    destino: obtenerValor(H.DESTINO),
    emailCliente: obtenerValor(H.EMAIL_CLIENTE),
    correoCC: obtenerValor(H.CORREO_CC),
    tipoProducto: obtenerValor(H.TIPO_PRODUCTO),
    ...
    tokens: {
      ...
      ASEGURADO_1: asegurados[0] || '',
      ASEGURADO_2: asegurados[1] || '',
      ASEGURADO_3: asegurados[2] || '',
      ASEGURADO_4: asegurados[3] || '',
      ASEGURADO_5: asegurados[4] || '',
...
function enviarWebhookPowerAutomate_(archivoHtml, data) {
  const payload = {
    ...
    tipoProducto: data.tipoProducto, // Individual / Grupal — columna AF
    aseguradosCount: data.numAsegurados,
 ...
 * MOCK (Roberto García, 82 años, Brasil).
 */
  ...
  try {
    ...
    const datosPrueba = {
      ...
      tokens: {
        ...
        FECHA_COTIZACION: Utilities.formatDate(hoy, Session.getScriptTimeZone(), "dd 'de' MMMM 'de' yyyy"),
        VIGENCIA: Utilities.formatDate(vigencia, Session.getScriptTimeZone(), "dd 'de' MMMM 'de' yyyy"),
        ASEGURADO_1: 'Roberto García, 82 años',
        ASEGURADO_2: '',
        ASEGURADO_3: '',
        ASEGURADO_4: '',
        ASEGURADO_5: '',
